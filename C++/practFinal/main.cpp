...
{
  //Count number of towns, cdps, and cities.
  int towncount, cdpcount, citycount;
  for (int i = 0; i < size; i++){
    if (data[i].type == "town")
      towncount++;
    if (data[i].type == "CDP")
  ...
  //Make new array for each of these.
  //Print out information asked for by user.
  for (int i = 0; i < size; i++){
    if (data[i].type == type){
  ...
  //Print out all the information sorted by type and population.
  cout << endl << "**** Results sorted by type and population ****" << endl;
  for (int i = 0; i < size; i++){
    if (data[i].type == "town") 
      cout << data[i].type << ": " << data[i].name << ", pop: "
	   << data[i].population << ", sq. miles: " << data[i].miles << endl;
