-- This file builds the networks model, only have used with the options linear and cosine -- If this file is ran standalone, command line argument must be passed in.
...
local noutputs = opt.noutput			-- # output nodes
local nfeatures = opt.nfeatures		-- # of features that each data point has
...
elseif opt.model == 'cos' then
  ...
  for i=1,nnodes do
    ...
    local weight = torch.Tensor(1, nfeatures)
    for j=1,nfeatures do
      weight[1][j] = torch.normal(0,stdDev)
    end 
    module.weight = weight
