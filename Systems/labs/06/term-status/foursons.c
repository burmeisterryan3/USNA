...
int main(){
  ...
  pid_t c_pid[4]; //array of 3 children pid's
  pid_t pid;
  int i, status;
  ...
  while ((pid = wait(&status)) > 0){
    if (WIFEXITED(status)){
      ...
      printf("Parent: Child %d: %d terminated and exited"
             " due to signal %s and also core dumped\n", i-1, pid, strsignal(WTERMSIG(status)));
    }
    i--;
  }
}
