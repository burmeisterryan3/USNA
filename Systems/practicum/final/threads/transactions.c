...
int main(){
  ...
  for(i=0;i<NUM_THREADS;i++){
    //TODO: Create all threads and pass transaction arguments
    //  (hint) : be mindful of shared memory
    pthread_mutex_init(&mutex, NULL);
    xargs = (struct ex_arguments *) calloc (1, sizeof(struct ex_arguments));
    xargs->n = NUM_TRANS/NUM_THREADS;
    if (i==0) {
      xargs->trans = &transactions[0];
    } else {
      xargs->trans = &transactions[i * xargs->n];
    }
