...
// DO NOT EDIT 
  ...
  while((client_sock = accept(server_sock, (struct sockaddr *) &client_saddr_in, &saddr_len)) > 0){
    ...
    if(connect(proxy_sock, (struct sockaddr *) &proxy_saddr_in, sizeof(proxy_saddr_in)) < 0){
      perror("connect");
      exit(1);
    }
    ...
    if((n = read(proxy_sock, buf, BUF_SIZE-1)) < 0){
      perror("read");
      exit(1);
    }
    ...
    if((n = read(client_sock, buf, BUF_SIZE-1)) < 0){
      perror("read");
      exit(1);
    }
    buf[n] = '\0'; //NULL terminate string
    ...
    if(write(proxy_sock, buf, strlen(buf)) < 0){
      perror("write");
      exit(1);
    }
