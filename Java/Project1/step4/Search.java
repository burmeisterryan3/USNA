...
public class Search
{
    ...
    public static void main(String[] args)
    {
        ...
        do {
            ...
            //Check to see if the first character of the input is a "-".  If so, remove tweets with the input
            //in the tweet text.
            else if (input.charAt(0) == '-'){
                input = input.substring(1);
                findWord = findWord.filterForNotKeyword(input);
            }
            //Check to see if date was entered, and if so, return queue with tweets on that date.
            else if (input.charAt(0) == '+'){
                input = input.substring(1);
